...
namespace BW.GateWay.Lottery.X5
{
    ...
    /// 顺子
    /// </summary>
    [BetChat(@"^(?<Type>[前中后])顺(?<Money>\d+)$")]
    public class Player185 : IX5
    {
        ...
        public override decimal Reward(string input, string number, decimal rewardMoney = decimal.Zero)
        {
            ...
            {
                int[] num = new int[] { resultNumber[i], resultNumber[i + 1], resultNumber[i + 2] };
                if (num.Distinct().Count() != 3) continue;
                if (num.Max() - num.Min() == 2) result.Add(this.InputBall[i]);
            }
