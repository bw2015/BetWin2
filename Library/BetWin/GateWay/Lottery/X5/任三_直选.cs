...
namespace BW.GateWay.Lottery.X5
{
    ...
    public class Player94 : IX5
    {
        ...
        public override int Bet(string input)
        {
            ...
            string[][] inputNumber = input.GetInputNumber(this.InputBall, new int[] { 1, 1, 1 }, new int[] { 10, 10, 10 });
            if (inputNumber == null) return 0;

            int flag = MathExtend.Combinations(3, flags.Length);
            int bet = inputNumber[0].Length * inputNumber[1].Length * inputNumber[2].Length;
            return flag * bet;
        }
            ...
            for (int i = 0; i < flags.Length; i++)
            {
                for (int n = i + 1; n < flags.Length; n++)
                {
                    for (int m = n + 1; m < flags.Length; m++)
                    {
                        ...
                        if (inputNumber[0].Contains(n1) && inputNumber[1].Contains(n2) && inputNumber[2].Contains(n3))
                        {
